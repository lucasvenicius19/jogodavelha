...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        if state.current_winner == other_player:
            # we should return position and score because we want to keep track of the score
            # for minimax to work
            return {
                "position": None,
                "score": 1 * (state.number_empty_squares() + 1) if other_player == max_player 
                else -1 * (state.number_empty_squares() + 1)
            }
